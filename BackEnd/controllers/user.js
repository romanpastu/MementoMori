...
async function updateUser(req, res) {
  ...
  const updateQuery = new PQ({ text: 'UPDATE users SET email = $1 , first_name = $2 , second_name = $3 where id = $4', values: [mail, firstName, secondName, userId] });
  db.query(updateQuery).then(async (data) => {
    if (password1 != '') {
      ...
      const updatePasswordQuery = new PQ({ text: 'UPDATE users SET password = $1 where id = $2', values: [pass, userId] });
...
async function getUserInfo(req, res) {
  ...
  const decoded = decode(token, { complete: true });
  const { userId } = decoded.payload;
  const selectInfo = new PQ({ text: 'SELECT * FROM users where id= $1', values: [userId] });
...
async function deleteUser(req, res) {
  ...
  const decoded = decode(token, { complete: true });
  const { userId } = decoded.payload;
  const deleteUser = new PQ({ text: 'DELETE FROM users where id = $1', values: [userId] });
